...
\toprule
\textbf{} & \multicolumn{4}{c|}{\textbf{within tropics $\pm$15$^{\circ}$}$^{1}$} & \multicolumn{4}{c|}{\textbf{total}$^{6}$} & \multicolumn{4}{c|}{\textbf{within tropics $\pm$10$^{\circ}$}} & \multicolumn{4}{c}{\textbf{within tropics $\pm$20$^{\circ}$}} \\
&&&&&&&&&&&&&&&& \\
\textbf{} & \multicolumn{2}{c}{\textbf{correlation}$^{2}$} & \multicolumn{2}{c|}{\textbf{\% overlap}$^{3}$} & \multicolumn{2}{c}{\textbf{correlation}} & \multicolumn{2}{c|}{\textbf{\% overlap}} & \multicolumn{2}{c}{\textbf{correlation}} & \multicolumn{2}{c|}{\textbf{\% overlap}} & \multicolumn{2}{c}{\textbf{correlation}} & \multicolumn{2}{c}{\textbf{\% overlap}} \\
\textbf{scenario} & \textbf{val.}$^{4}$ & \textbf{p-val.}$^{5}$ & \textbf{val.} & \textbf{p-val.} & \textbf{val.} & \textbf{p-val.} & \textbf{val.} & \textbf{p-val.} & \textbf{val.} & \textbf{p-val.} & \textbf{val.} & \textbf{p-val.} & \textbf{val.} & \textbf{p-val.} & \textbf{val.} & \textbf{p-val.} \\
\midrule
...
\begin{tablenotes}[para,flushleft]
\vspace{0.15cm}
$^{1}$our preferred definition of the tropical rain belt.
\vspace{0.15cm}
...
$^{5}$`p-val.' refers to the fraction of randomly timed glacial interval simulations that correlate/overlap better with LIP area than the actual ice extent record (i.e. the p-value with respect to the null hypothesis of no correlation/overlap).
\vspace{0.15cm}
...
\begin{center}
	...
	\caption{\textbf{A)} LIPs included in this analysis. The size of each circle reflects the initial area estimate of each LIP. The + indicates the timing and absolute paleolatitude of the centroid of each LIP at the time of emplacement. Car.-Col. = Caribbean-Colombian. \textbf{B)} Total LIP area through time for the different post-emplacent models. Only the `no decay' model excludes pre-Phanerozoic LIPs. \textbf{C)} Tropical LIP area through time for the different post-emplacement models. The arrows to the right indicate reconstructed tropical LIP area at the time of emplacement for the ca. 720 and 1109~Ma Franklin and Umkondo LIPs. The triangles show the paleogeographic reconstruction times in Fig. \ref{fig:Reconstruction_Snapshots}. \textbf{D)} Heat map showing the latitudinal distribution of LIP area for the preferred post-emplacement model. \textbf{E)} Latitudinal extent of land ice away from the poles through time \citep{Macdonald2019a} and compilation of \textit{p}CO$_{2}$ proxies \citep{Foster2017a} (\textit{p}CO$_{2}$ y-axis reversed, and in log-scale). Error bars indicate standardized uncertainties, and grey bands indicate 68 and 95\% confidence intervals for Monte Carlo resampled LOESS fits through the \textit{p}CO$_{2}$ proxy data.}
	\label{fig:LIP_Areas}
\end{center}
\end{figure}
...
\begin{center}
	...
	\caption{Paleogeographic reconstructions for times that correspond to peaks of LIP area in the tropics (Fig. \ref{fig:LIP_Areas}C). The opacity of LIP polygons indicates their interpreted remainingu area at the time of the reconstruction as a percentage of initial LIP area, under the preferred post-emplacement model of `t$_{1/2}$ = 36~Myr + 50\% burial'.}
